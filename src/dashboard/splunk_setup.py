...
def send_alert(subject, message):
    executor.submit(send_email_alert, subject, message)
    executor.submit(send_telegram_alert, message)
    executor.submit(log_to_splunk, {"subject": subject, "message": message})
            ...
            with open(log_file, "r") as f:
                for line in f:
                    if line not in seen_entries:
                        ...
                        executor.submit(honeypot_alert_handler, line, honeypot_type)
        except FileNotFoundError:
            logging.error(f"{log_file} not found.")
        time.sleep(5)
